...
class StockAssignmentService:
    ...
    def get_all(self):
        return self.db.query(database_models.StockAssignment).all()
    
    def get_by_id(self, assignment_id: int):
        return self.db.query(database_models.StockAssignment).filter(
            database_models.StockAssignment.id == assignment_id
        ).first()
